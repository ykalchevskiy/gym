...
class SegmentTreeEx(object):
    ...
    def _query(self, v, tl, tr, l, r):
        ...
        if tl == tr:
            ...
            tm = (tl + tr) // 2
            return self.tree[v] + max([
                self._query(v * 2, tl, tm, l, min(r, tm)),
                self._query(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r)
            ])

    def inc(self, l, r, value):
        self._inc(1, 0, len(self.array) - 1, l, r, value)

    def _inc(self, v, tl, tr, l, r, value):
        if l > r:
            return
        if l == tl and r == tr:
            self.tree[v] += value
            ...
            tm = (tl + tr) // 2
            self._inc(v * 2, tl, tm, l, min(r, tm), value)
            self._inc(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, value)
